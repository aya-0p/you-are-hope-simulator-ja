...
//データを設定
const projectData: Map<string, PointData> = new Map()
...
const imageItems: Map<string, Items> = (() => {
  ...
  imageItemsOriginal.forEach(item => {
    ...
    img.addEventListener('load', () => {
      imageLoadStatus++
      if (imageItemsOriginal.length === imageLoadStatus) allLoaded()
    }, false);
  })
...
const allLoaded = () => {
  ...
  const d: Map<string, PointData> = (() => {
    const r = new Map()
    try {
      if (!data) return r
      const t: Array<{ name: string, data: PointData }> = JSON.parse(JSON.parse(data))
      t.forEach(e => {
        r.set(e.name, e.data)
      })
      return r
...
const draw = () => {
  ...
  projectData.forEach(data => {
    if (data.layer1) {
      ...
      if (drawData) ctx.drawImage(drawData.img, ((data.x * settings.block) - drawData.center.x + 64) * settings.zoom, ((data.y * settings.block) - drawData.center.y + 64) * settings.zoom, drawData.width * settings.zoom, drawData.height * settings.zoom)
    } else {
...
const save = () => {
  const saves: Array<{ name: string, data: PointData }> = []
...
const fillBiome = (biome?: Biome) => {
  projectData.forEach(data => {
    if (biome) data.layer0 = `${biome}_${Math.floor(Math.random() * 32 + 1)}`
    else data.layer0 = `biome_unknown_${Math.floor(Math.random() * 16 + 1)}`
  })
}

//context menu
window.addEventListener("DOMContentLoaded", () => {
  const menu: Array<ContextMenus> = [
    {
      name: "グリッドの表示を切り替え",
  ...
  const CMenu = (Proto as Proto_).Menu
  //new CMenu({
  const menuObj = (CMenu as ContextMenu)({
    selector: "canvas",
    className: "menu google",
    menuItems: menu
  })
  const subMenuData: Array<subMenuD> = [
    {
      point: 2,
      items: [
        {
...
const changeData = (x: number, y: number) => {
  const clickX = Math.floor(x / (settings.block * settings.zoom))
  const clickY = Math.floor(y / (settings.block * settings.zoom))
  if (settings.clickedList.includes(`${clickX}_${clickY}`)) return
  else settings.clickedList.push(`${clickX}_${clickY}`)
  ...
  switch (settings.currentSelectingType) {
    ...
    case "biome_yellow_prairies":
      ...
      break;
    case "delete":
      if (pointData.layer4) pointData.layer4 = undefined
      else if (pointData.layer3) pointData.layer3 = undefined
      else if (pointData.layer2) pointData.layer2 = undefined
      else if (pointData.layer1) pointData.layer1 = undefined
      break;
    case "wall":
...
document.getElementById('canvas')?.addEventListener("mousemove", (event) => {
  if (event.button === 0) {
    if (settings.isClick) changeData(event.x, event.y)
  }
})
...
type Category =
  ...
  "biome_swamps" |
  "biome_tundra" |
  "biome_yellow_prairies" |
  "floor" |
  "wall"
type Biome = "biome_grasslands" |
  "biome_badlands" |
  "biome_desert" |
  "biome_jungle" |
  "biome_swamps" |
...
type ContextMenus = ContextShownMenu | ContextDisabledMenu | ContextSeparator
interface ContextShownMenu {
  /**表示名 */
  name: string;
  /**css class名 */
  className?: string;
...
type ContextMenu = (param: ContextMenuConstructorParam) => { container: HTMLDivElement }
interface ContextMenuConstructorParam {
  selector: string;
  className: "menu desktop" | "menu firefox" | "menu google";
  menuItems: Array<ContextMenus>
  beforeShow?: (event: any) => void;
  beforeHide?: (event: any) => void;
  beforeSelect?: (event: any) => void;
}
interface subMenuI {
  name: string;
